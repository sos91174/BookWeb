	...
	<Script type="text/javascript">
		$(function () {
			...
			$("button.addToCart").click(function () {
				/**
				 * 在事件響應的function函数中，有一个this對象，這個this對象，是當前正在響應事件的dom對象
				 * @type {jQuery}
				 */
...
<body>
	...
	<div id="header">
			...
			<span class="wel_word">線上購物</span>
			<div>
				<%--如果用戶還没有登入，則顯示 【登入 和註冊的目錄】 --%>
				...
				<c:if test="${not empty sessionScope.user}">
					<span>歡迎光臨<span class="um_span">${sessionScope.user.username}</span>商城</span>
		...
		<div id="book">
			...
			<div style="text-align: center">
				<c:if test="${empty sessionScope.cart.items}">
					...
					<span> </span>
					<div>
						<span style="color: red">目前購物車是空的</span>
					</div>
				</c:if>
				<c:if test="${not empty sessionScope.cart.items}">
					<%--購物車如果不是空的話輸出--%>
					<span>您的購物車中有 ${sessionScope.cart.totalCount} 件商品</span>
					<div>
						您剛剛將<span style="color: red">${sessionScope.lastName}</span>加入到了購物車中
					</div>
				</c:if>
			</div>
			...
			<div class="b_list">
				...
				<div class="book_info">
					...
					<div class="book_sales">
						<span class="sp1">銷貨量:</span>
						<span class="sp2">${book.sales}</span>
					</div>
					<div class="book_amount">
						<span class="sp1">庫存量:</span>
